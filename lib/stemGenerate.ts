...
function parseOutput(stdout: string): StemGenerateResult {
  const last =
    stdout
      .split("\n")
      .map((line) => line.trim())
  ...
  const parsed = JSON.parse(last) as StemGenerateResult & {
    ok?: boolean;
    error?: string;
  };
  if (!parsed.ok) {
    throw new Error(parsed.error ?? "Error desconocido al separar instrumentos");
  }
  return parsed;
}
...
export async function generateStemsFromSource(
  ...
  const scriptPath = path.join(process.cwd(), "scripts", "separate_stems.py");
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      try {
        ...
        reject(
          new Error(
            payload.error ?? stderr.trim() ?? `Separación fallida (código ${code})`,
          ),
        );
      } catch {
        reject(
          new Error(stderr.trim() || stdout.trim() || `Separación fallida (código ${code})`),
        );
      }
    });
  });
